...
function showQuestion(id) {
        ...
        if (id <= totalQuestions) {
			...
			getEndingSentence() //begins the end screen process if id is above total question
        }
    }
...
function getEndingSentence() {
    ...
    var quizRadio1 = document.getElementsByName("dq");
    var quizRadio2 = document.getElementsByName("cq");
    var quizRadio3 = document.getElementsByName("aq");
    var quizRadio4 = document.getElementsByName("bq");
  	var content = ''; //It's easier to handle if we simply merge all sentences into a string
    var content1 = ''; //It's easier to handle if we simply merge all sentences into a string
    var content2 = ''; //It's easier to handle if we simply merge all sentences into a string
    var content3 = ''; //It's easier to handle if we simply merge all sentences into a string
    var content4 = ''; //It's easier to handle if we simply merge all sentences into a string
    ...
    for (var i = 0; i < quizRadio1.length; i++) {
        if (quizRadio1[i].checked) {
            content1 += quizRadio1[i].getAttribute("data-endingsentence"); //these are the attributes used to generate quiz answers
        }
    }
    for (var i = 0; i < quizRadio2.length; i++) {
        if (quizRadio2[i].checked) {
            content2 += quizRadio2[i].getAttribute("data-endingsentence"); //these are the attributes used to generate quiz answers
        }
    }
    for (var i = 0; i < quizRadio3.length; i++) {
        if (quizRadio3[i].checked) {
            content3 += quizRadio3[i].getAttribute("data-endingsentence"); //these are the attributes used to generate quiz answers
        }
    }
    for (var i = 0; i < quizRadio4.length; i++) {
        if (quizRadio4[i].checked) {
            content4 += quizRadio4[i].getAttribute("data-endingsentence"); //these are the attributes used to generate quiz answers
        }
    }
	// return content;
  personalityResults = content + ", " + content1 + ", " + content2 + ", " + content3 + ", " + content4;
